using System;
using System.Text.Json.Serialization;
using ModelingEvolution.JsonParsableConverter;

namespace ModelingEvolution.Ipv4;

/// <summary>
/// Represents a TCP/UDP port number (0-65535).
/// </summary>
/// <remarks>
/// <para>
/// This struct provides a strongly-typed representation of network ports with
/// built-in validation, well-known port constants, and range classification.
/// It implements <see cref="IParsable{TSelf}"/> for seamless JSON serialization.
/// </para>
/// </remarks>
/// <example>
/// <code>
/// // Create ports
/// var port1 = new IpPort(8080);
/// var port2 = IpPort.Parse("443");
/// var port3 = IpPort.Http;
///
/// // Properties
/// bool isWellKnown = port1.IsWellKnown;     // false (8080 > 1023)
/// bool isRegistered = port1.IsRegistered;   // true (1024-49151)
/// bool isDynamic = port1.IsDynamic;         // false
///
/// // Implicit conversions
/// ushort value = port1;  // 8080
/// IpPort port4 = 3000;   // implicit from int
///
/// // Well-known ports
/// var http = IpPort.Http;        // 80
/// var https = IpPort.Https;      // 443
/// var ssh = IpPort.Ssh;          // 22
/// </code>
/// </example>
[JsonConverter(typeof(JsonParsableConverter<IpPort>))]
public readonly record struct IpPort : IParsable<IpPort>, IComparable<IpPort>
{
    private readonly ushort _value;

    #region Well-Known Ports (0-1023)

    /// <summary>FTP Data (20)</summary>
    public static IpPort FtpData { get; } = new(20);

    /// <summary>FTP Control (21)</summary>
    public static IpPort Ftp { get; } = new(21);

    /// <summary>SSH (22)</summary>
    public static IpPort Ssh { get; } = new(22);

    /// <summary>Telnet (23)</summary>
    public static IpPort Telnet { get; } = new(23);

    /// <summary>SMTP (25)</summary>
    public static IpPort Smtp { get; } = new(25);

    /// <summary>DNS (53)</summary>
    public static IpPort Dns { get; } = new(53);

    /// <summary>DHCP Server (67)</summary>
    public static IpPort DhcpServer { get; } = new(67);

    /// <summary>DHCP Client (68)</summary>
    public static IpPort DhcpClient { get; } = new(68);

    /// <summary>TFTP (69)</summary>
    public static IpPort Tftp { get; } = new(69);

    /// <summary>HTTP (80)</summary>
    public static IpPort Http { get; } = new(80);

    /// <summary>Kerberos (88)</summary>
    public static IpPort Kerberos { get; } = new(88);

    /// <summary>POP3 (110)</summary>
    public static IpPort Pop3 { get; } = new(110);

    /// <summary>NTP (123)</summary>
    public static IpPort Ntp { get; } = new(123);

    /// <summary>NetBIOS Name Service (137)</summary>
    public static IpPort NetBiosNs { get; } = new(137);

    /// <summary>NetBIOS Session Service (139)</summary>
    public static IpPort NetBiosSs { get; } = new(139);

    /// <summary>IMAP (143)</summary>
    public static IpPort Imap { get; } = new(143);

    /// <summary>SNMP (161)</summary>
    public static IpPort Snmp { get; } = new(161);

    /// <summary>LDAP (389)</summary>
    public static IpPort Ldap { get; } = new(389);

    /// <summary>HTTPS (443)</summary>
    public static IpPort Https { get; } = new(443);

    /// <summary>SMB (445)</summary>
    public static IpPort Smb { get; } = new(445);

    /// <summary>SMTP over TLS (465)</summary>
    public static IpPort SmtpTls { get; } = new(465);

    /// <summary>LDAPS (636)</summary>
    public static IpPort Ldaps { get; } = new(636);

    /// <summary>SMTP Submission (587)</summary>
    public static IpPort SmtpSubmission { get; } = new(587);

    /// <summary>IMAPS (993)</summary>
    public static IpPort Imaps { get; } = new(993);

    /// <summary>POP3S (995)</summary>
    public static IpPort Pop3s { get; } = new(995);

    #endregion

    #region Common Registered Ports (1024-49151)

    /// <summary>SOCKS Proxy (1080)</summary>
    public static IpPort Socks { get; } = new(1080);

    /// <summary>OpenVPN (1194)</summary>
    public static IpPort OpenVpn { get; } = new(1194);

    /// <summary>MS SQL Server (1433)</summary>
    public static IpPort MsSql { get; } = new(1433);

    /// <summary>Oracle Database (1521)</summary>
    public static IpPort Oracle { get; } = new(1521);

    /// <summary>MySQL (3306)</summary>
    public static IpPort MySql { get; } = new(3306);

    /// <summary>RDP (3389)</summary>
    public static IpPort Rdp { get; } = new(3389);

    /// <summary>PostgreSQL (5432)</summary>
    public static IpPort PostgreSql { get; } = new(5432);

    /// <summary>VNC (5900)</summary>
    public static IpPort Vnc { get; } = new(5900);

    /// <summary>Redis (6379)</summary>
    public static IpPort Redis { get; } = new(6379);

    /// <summary>HTTP Proxy (8080)</summary>
    public static IpPort HttpProxy { get; } = new(8080);

    /// <summary>HTTPS Alternate (8443)</summary>
    public static IpPort HttpsAlt { get; } = new(8443);

    /// <summary>Elasticsearch HTTP (9200)</summary>
    public static IpPort Elasticsearch { get; } = new(9200);

    /// <summary>MongoDB (27017)</summary>
    public static IpPort MongoDb { get; } = new(27017);

    #endregion

    /// <summary>
    /// Initializes a new instance of the <see cref="IpPort"/> struct.
    /// </summary>
    /// <param name="value">The port number (0-65535).</param>
    public IpPort(ushort value)
    {
        _value = value;
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="IpPort"/> struct.
    /// </summary>
    /// <param name="value">The port number (must be 0-65535).</param>
    /// <exception cref="ArgumentOutOfRangeException">Thrown when value is outside 0-65535 range.</exception>
    public IpPort(int value)
    {
        ArgumentOutOfRangeException.ThrowIfNegative(value);
        ArgumentOutOfRangeException.ThrowIfGreaterThan(value, ushort.MaxValue);
        _value = (ushort)value;
    }

    /// <summary>
    /// Gets the port number value.
    /// </summary>
    public ushort Value => _value;

    /// <summary>
    /// Gets whether this is a well-known port (0-1023).
    /// </summary>
    /// <remarks>
    /// Well-known ports are assigned by IANA and typically require root/admin privileges to bind.
    /// </remarks>
    public bool IsWellKnown => _value <= 1023;

    /// <summary>
    /// Gets whether this is a registered port (1024-49151).
    /// </summary>
    /// <remarks>
    /// Registered ports are assigned by IANA for specific services but don't require special privileges.
    /// </remarks>
    public bool IsRegistered => _value >= 1024 && _value <= 49151;

    /// <summary>
    /// Gets whether this is a dynamic/ephemeral port (49152-65535).
    /// </summary>
    /// <remarks>
    /// Dynamic ports are used for temporary connections and are typically assigned by the OS.
    /// </remarks>
    public bool IsDynamic => _value >= 49152;

    /// <summary>
    /// Returns the string representation of this port.
    /// </summary>
    public override string ToString() => _value.ToString();

    /// <summary>
    /// Parses a string representation of a port number.
    /// </summary>
    /// <param name="s">A string containing a port number.</param>
    /// <param name="provider">An object that provides culture-specific formatting information (not used).</param>
    /// <returns>A <see cref="IpPort"/> equivalent to the number contained in <paramref name="s"/>.</returns>
    /// <exception cref="FormatException">Thrown when <paramref name="s"/> is not a valid port number.</exception>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="s"/> is null.</exception>
    public static IpPort Parse(string s, IFormatProvider? provider = null)
    {
        ArgumentNullException.ThrowIfNull(s);

        return TryParse(s, provider, out var result)
            ? result
            : throw new FormatException($"Invalid port number: '{s}'");
    }

    /// <summary>
    /// Attempts to parse a string representation of a port number.
    /// </summary>
    /// <param name="s">A string containing a port number.</param>
    /// <param name="provider">An object that provides culture-specific formatting information (not used).</param>
    /// <param name="result">When this method returns, contains the parsed port if successful.</param>
    /// <returns><c>true</c> if parsing succeeded; otherwise, <c>false</c>.</returns>
    public static bool TryParse(string? s, IFormatProvider? provider, out IpPort result)
    {
        result = default;

        if (string.IsNullOrWhiteSpace(s))
            return false;

        if (!ushort.TryParse(s.Trim(), out var value))
            return false;

        result = new IpPort(value);
        return true;
    }

    /// <summary>
    /// Compares this instance with another <see cref="IpPort"/>.
    /// </summary>
    public int CompareTo(IpPort other) => _value.CompareTo(other._value);

    /// <summary>
    /// Implicitly converts an <see cref="IpPort"/> to a <see cref="ushort"/>.
    /// </summary>
    public static implicit operator ushort(IpPort port) => port._value;

    /// <summary>
    /// Implicitly converts a <see cref="ushort"/> to an <see cref="IpPort"/>.
    /// </summary>
    public static implicit operator IpPort(ushort value) => new(value);

    /// <summary>
    /// Implicitly converts an <see cref="int"/> to an <see cref="IpPort"/>.
    /// </summary>
    /// <exception cref="ArgumentOutOfRangeException">Thrown when value is outside 0-65535 range.</exception>
    public static implicit operator IpPort(int value) => new(value);

    /// <summary>
    /// Implicitly converts a string to an <see cref="IpPort"/>.
    /// </summary>
    public static implicit operator IpPort(string value) => Parse(value, null);
}
